<?php
    class Connection{
        ...
        public function selectGroupBy($table,$fields,$groupby){
            $fieldNames = NULL;
            ...
            $sql="SELECT $fieldNames FROM $table GROUP BY $groupby";
            $result = mysqli_query($this->conn, $sql);
            $finale=array();
            if (mysqli_num_rows($result) > 0){
                while($row = mysqli_fetch_assoc($result)){
        ...
        public function joindelete($table,$where){
            $sql="DELETE $table FROM $table WHERE $where";
            mysqli_query($this->conn, $sql);
            if(mysqli_affected_rows($this->conn)>0){
				return true;
			}else{
        ...
        public function quaryexe($quary){
            $sql=$quary;
            $result = mysqli_query($this->conn, $sql);
            $finale=array();
            if (mysqli_num_rows($result) > 0){
